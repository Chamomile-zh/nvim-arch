...
local function apply_colors()
  ...
  vim.api.nvim_set_hl(0, 'DIY_RainbowMatch', { underline = true })
end
...
local function update_rainbow(buf)
  ...
  for id, node in query:iter_captures(root, buf, 0, -1) do
    ...
    if ok_text and type(text) == 'string' then
      ...
      elseif bracket_pairs[text] then
        local color_idx = (depth - 1) % #hl_targets + 1
        pcall(vim.api.nvim_buf_set_extmark, buf, ns, sr, sc, {
          end_row = er,
          end_col = ec,
        ...
        for i = #stack, 1, -1 do
          if stack[i].char == bracket_pairs[text] then
            ...
            if map_cache[match.r] and map_cache[match.r][match.c] then
...
function M.setup()
  ...
  local augroup = vim.api.nvim_create_augroup('DIY_TSRainbow', { clear = true })
